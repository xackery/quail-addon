...
def obj_export(obj: bpy.types.Object, path: str):
    print("  Mesh", obj.name)
    mesh = obj.to_mesh()
    material_export(mesh.materials, path)
    with open("%s/%s_vertex.txt" % (path, obj.name), "w") as vw:
        ...
        for i in range(len(mesh.vertices)):
            ...
            vw.write("%0.3f,%0.3f,%0.3f|%0.3f,%0.3f,%0.3f|%0.3f,%0.3f|%0.3f,%0.3f|%0.3f,%0.3f,%0.3f,%0.3f\n" % (
...
def material_export(materials: bpy.types.IDMaterials, path: str):
    with open(path+"/material.txt", "w") as mw:
        mw.write("id|name|flag|shader_name\n")
        for mat in materials:
            ...
            with open(path+"/material_property.txt", "w") as mw:
                ...
                if mat.node_tree.nodes["Principled BSDF"].inputs[7].default_value != 0:
                    mw.write("%s|%s|%f|%d\n" % (
                        mat.name, "e_fShininess0", mat.node_tree.nodes["Principled BSDF"].inputs[7].default_value, 2))
